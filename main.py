...
bot_state = {
    ...
    "market_regime": {s: "UNKNOWN" for s in SYMBOLS},
    "version": "ForexVPA-1.2"
}
...
def check_symbol_regime(symbol):
    """Check each pair's own 200-day EMA trend independently.
    For USD/JPY: price ABOVE 200 EMA = uptrend = OK to buy.
    For EUR/USD and GBP/USD: price ABOVE 200 EMA = uptrend = OK to buy.
    Each pair is judged on its own merits, not on another pair's trend."""
    try:
        candles = get_candles(symbol, "D", 210)
        if len(candles) < 200:
            log.warning(f"{symbol} not enough daily bars for regime check — defaulting to UNKNOWN")
            return "UNKNOWN"
        ...
        if not ema200:
            return "UNKNOWN"
        price = closes[-1]
        regime = "BULL" if price > ema200[-1] else "BEAR"
        log.info(f"Regime {symbol}: {regime} | price={round(price,5)} | 200EMA={round(ema200[-1],5)}")
        return regime
    except Exception as e:
...
def trading_loop():
    add_diary("SYSTEM", "ForexAI VPA Bot v1.2 started | Per-symbol regime filter | Vol spike: 1.5x | Min score: 3 | SL=15pips | TP=30pips", "system")
    log.info("ForexAI VPA Bot v1.2 started")
    ...
    while True:
        try:
            ...
            # Check each symbol's own regime every 30 minutes
            ...
            for symbol in SYMBOLS:
                ...
                log.info(f"{symbol} | price={price} vol_ratio={sig['vol_ratio']} BUY={sig['buy_score']} SELL={sig['sell_score']} signals={sig['signals']} regime={bot_state['market_regime'][symbol]}")
                ...
                if symbol in bot_state["positions"]:
                    ...
                    # Each pair checked against its OWN regime
                    ...
                    if sig["buy_score"] >= STRATEGY["min_score"] and sig["buy_score"] > sig["sell_score"] and regime_ok:
                        ...
                        log.info(f"{symbol} | BEAR regime — entry blocked")
...
def index():
    try:
        ...
        return jsonify({"status": "ForexAI VPA Bot v1.2 running"})
